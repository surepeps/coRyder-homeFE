...
function SureModal() {
    ...
    const backgroundImageStyle = modalOptions.modalBackgroundImg
        ? {
              backgroundImage: `url(${modalOptions.modalBackgroundImg})`,
              backgroundSize: 'cover',
              backgroundPosition: 'center',
          }
        : {};

    const modalClasses = `fixed ${isRequestLoading ? 'z-40' : 'z-50'} p-4 overflow-x-hidden flex items-center justify-center w-full overflow-y-auto inset-0 h-[calc(100%-1rem)] h-full bg-black bg-opacity-50 ${isOpen ? '' : 'opacity-0 pointer-events-none'}`;

    const closeButtonClasses = `absolute top-3 right-2.5 hover:bg-[#BF040120] transition duration-300 ease-in-out rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center`;
    ...
    return (
        ...
        >
            <div className={`relative w-full ${modalSize} max-h-full animate-fadeIn`}>
                <div className={`relative ${bgColor} justify-between rounded-3xl shadow`} style={backgroundImageStyle}>
                    <div className="topArea">
                        {modalOptions.modalTopImg && (
                            <img src={modalOptions.modalTopImg} className='w-full h-auto' alt="" />
                        )}
